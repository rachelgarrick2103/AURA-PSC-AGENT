...
const SEGMENT_META = {
  all:      { label:"All",          color:"#fff",  bg:"#111", count:9 },
  hot_lead: { label:"Hot Leads",    color:"#fff",  bg:"#333", count:2 },
  gone_quiet:{ label:"Gone Quiet",  color:"#fff",  bg:"#333", count:3 },
  lapsed:   { label:"Lapsed",       color:"#fff",  bg:"#333", count:2 },
  promo:    { label:"Promo Targets",color:"#fff",  bg:"#333", count:2 },
};
...
const FAQS = [
  ...
  { q:"Can clients still reach a human?", a:"Absolutely. AURA hands off to you via text alert if a client specifically asks or if she genuinely can't resolve something." },
  ...
  return (
    <div style={{ fontFamily: DISPLAY, background:"#000", color:"#fff", overflowX:"hidden" }}>
      <style>{`
        ...
        .gbtn:hover{background:rgba(255,255,255,.06)!important}
        .hlift{transition:transform .2s}
        .hlift:hover{transform:translateY(-3px)}
      ...
      {/* NAV */}
      <nav style={{ position:"sticky",top:0,zIndex:100,background:"rgba(0,0,0,.9)",backdropFilter:"blur(14px)",borderBottom:"1px solid #1c1c1c",padding:"0 52px",height:58,display:"flex",alignItems:"center" }}>
        ...
        <div style={{ maxWidth:1200,margin:"0 auto",width:"100%",display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"center" }}>
          ...
          <div>
            ...
            <div className="fu d3" style={{ display:"flex",gap:12 }}>
              ...
              <button onClick={() => setView("dashboard")} className="gbtn" style={{ background:"transparent",color:"#666",border:"1px solid #252525",padding:"14px 22px",borderRadius:7,fontSize:15,fontFamily:SANS,cursor:"pointer" }}>See Dashboard →</button>
            </div>
    ...
    <div style={{ fontFamily:SANS,background:"#f7f7f5",minHeight:"100vh",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ display:"flex",flex:1,overflow:"hidden" }}>
        ...
        <div style={{ width:196,background:"#fff",borderRight:"1px solid #ebebeb",padding:"16px 10px",display:"flex",flexDirection:"column",gap:2 }}>
          {NAV.map(n => (
            <div key={n.id} className="db-item si" onClick={() => setTab(n.id)} style={{ display:"flex",gap:10,alignItems:"center",padding:"9px 12px",background:tab===n.id?"#000":"transparent",color:tab===n.id?"#fff":"#555",position:"relative" }}>
              <span style={{ fontSize:13 }}>{n.icon}</span>
              <span style={{ fontSize:13,fontWeight:tab===n.id?500:400 }}>{n.label}</span>
              {n.id==="sales" && <span style={{ marginLeft:"auto",background:tab==="sales"?"rgba(255,255,255,.2)":"#111",color:tab==="sales"?"#fff":"#fff",fontSize:10,fontWeight:600,padding:"1px 7px",borderRadius:100 }}>9</span>}
            </div>
          ))}
          <div style={{ flex:1 }} />
...
function SalesAgentTab() {
  ...
  const filtered = SALES_CLIENTS.filter(c => segment==="all" || c.segment===segment).filter(c => !skipped[c.id]);

  const segmentColor = (s) => ({ hot_lead:"#111", gone_quiet:"#333", lapsed:"#555", promo:"#777" }[s] || "#888");
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens:300,
          system:`You write short, warm, personalised outreach messages for a beauty studio owner to send to clients via WhatsApp or SMS. 
...
If they're a lapsed client: acknowledge the time, make it feel warm not guilt-trippy.
...
Return ONLY the message text. No subject line, no signature label, no preamble.`,
          messages:[{ role:"user", content:`Client name: ${client.name.split(" ")[0]}
Situation: ${client.context}
Interested in: ${client.service}
Source of contact: ${client.source}
Segment: ${client.label}
Studio owner name: Rachel
Studio name: Glow Studio

Write the outreach message now.`}]
        })
      });
      const data = await res.json();
      const msg = data.content?.find(b=>b.type==="text")?.text?.trim() || "Couldn't generate — try again.";
      setDrafts(d => ({...d, [client.id]:msg}));
    } catch {
      setDrafts(d => ({...d, [client.id]:"Error generating message. Check connection and try again."}));
    }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:24 }}>
        <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between" }}>
          <div>
            ...
            <p style={{ fontFamily:SANS,fontSize:13,color:"#999",lineHeight:1.6 }}>
              AURA identifies clients to re-engage and drafts personalised outreach for you to approve and send.
            </p>
          </div>
          <div style={{ display:"flex",gap:10 }}>
            {[
              { val:`${sentCount}`, label:"Sent today" },
              { val:`${filtered.length}`, label:"In queue" },
              { val:"—", label:"Replies" },
            ].map(s => (
      ...
      <div style={{ display:"grid",gridTemplateColumns:"180px 1fr",gap:16 }}>
        ...
        {/* Segment sidebar */}
        <div style={{ background:"#fff",border:"1px solid #ebebeb",borderRadius:12,padding:12,height:"fit-content",position:"sticky",top:0 }}>
          ...
          {segs.map(s => (
            <button key={s.id} className="si" onClick={() => setSegment(s.id)} style={{ width:"100%",background:segment===s.id?"#000":"transparent",color:segment===s.id?"#fff":"#555",border:"none",padding:"9px 10px",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",fontFamily:SANS,fontSize:13,fontWeight:segment===s.id?500:400,borderRadius:8,marginBottom:2 }}>
              {s.label}
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
          ...
          {filtered.map(client => (
            <div key={client.id} style={{ background:"#fff",border:`1px solid ${sent[client.id]?"#d0d0d0":"#ebebeb"}`,borderRadius:12,overflow:"hidden",opacity:sent[client.id]?.75:1,transition:"opacity .3s" }}>

              {/* Card header */}
              <div style={{ padding:"16px 20px",display:"flex",alignItems:"center",gap:14 }}>
                <div style={{ width:40,height:40,borderRadius:"50%",background:sent[client.id]?"#f0f0f0":"#111",display:"flex",alignItems:"center",justifyContent:"center",color:sent[client.id]?"#999":"#fff",fontFamily:SANS,fontSize:12,fontWeight:600,flexShrink:0 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:3 }}>
                    ...
                    <span style={{ fontFamily:SANS,fontSize:10,background:segmentColor(client.segment),color:"#fff",padding:"2px 9px",borderRadius:100,fontWeight:500,letterSpacing:".06em",textTransform:"uppercase" }}>{client.label}</span>
                    {client.priority==="high" && !sent[client.id] && <span style={{ fontFamily:SANS,fontSize:10,color:"#999" }}>↑ Priority</span>}
                    {sent[client.id] && <span style={{ fontFamily:SANS,fontSize:11,color:"#aaa" }}>Message sent</span>}
                  </div>
                  <div style={{ fontFamily:SANS,fontSize:12,color:"#999",lineHeight:1.6 }}>{client.context}</div>
                  <div style={{ fontFamily:SANS,fontSize:11,color:"#bbb",marginTop:4 }}>Interested in: <span style={{ color:"#888" }}>{client.service}</span> · Source: <span style={{ color:"#888" }}>{client.source}</span></div>
                </div>

                {/* Actions */}
                {!sent[client.id] && (
                  <div style={{ display:"flex",gap:8,flexShrink:0 }}>
                    <button onClick={() => draftMessage(client)} style={{ background:"#111",color:"#fff",border:"none",padding:"9px 16px",borderRadius:7,fontSize:12,cursor:"pointer",fontFamily:SANS,fontWeight:500,whiteSpace:"nowrap",transition:"background .15s" }}
                      onMouseOver={e=>e.target.style.background="#333"} onMouseOut={e=>e.target.style.background="#111"}>
                      {loading[client.id] ? "Drafting..." : drafts[client.id] ? "Re-draft" : "Draft Message"}
                    </button>
                    <button onClick={() => skip(client.id)} style={{ background:"transparent",color:"#ccc",border:"1px solid #eee",padding:"9px 14px",borderRadius:7,fontSize:12,cursor:"pointer",fontFamily:SANS }}>Skip</button>
                  </div>
                )}
              </div>
              ...
              {expanded===client.id && (loading[client.id] || drafts[client.id]) && !sent[client.id] && (
                <div style={{ borderTop:"1px solid #f0f0f0",padding:"16px 20px 20px",background:"#fafafa" }}>
                  ...
                  {loading[client.id] ? (
                    <div className="loading" style={{ fontFamily:SANS,fontSize:14,color:"#ccc",lineHeight:1.8,background:"#f0f0f0",borderRadius:8,padding:"14px 16px" }}>
                      AURA is writing a personalised message for {client.name.split(" ")[0]}...
                    </div>
                  ) : (
                    <div className="msg-in">
                      <textarea
                        defaultValue={drafts[client.id]}
                      ...
                      <div style={{ display:"flex",gap:10,marginTop:12,alignItems:"center" }}>
                        <button onClick={() => approve(client.id)} style={{ background:"#111",color:"#fff",border:"none",padding:"10px 22px",borderRadius:7,fontSize:13,cursor:"pointer",fontFamily:SANS,fontWeight:500,transition:"background .15s" }}
                          onMouseOver={e=>e.target.style.background="#333"} onMouseOut={e=>e.target.style.background="#111"}>
                          Approve & Send
                        </button>
                        ...
                        <div style={{ marginLeft:"auto",fontFamily:SANS,fontSize:11,color:"#ccc" }}>Edit before sending · Changes save automatically</div>
                      </div>
                    </div>
                  )}
                </div>
...
function TodayTab() {
  return (
    <div>
      <h1 style={{ fontFamily:DISPLAY,fontSize:26,fontWeight:400,color:"#111",marginBottom:4,letterSpacing:".02em" }}>Good morning, Rachel</h1>
      ...
      <div style={{ background:"#fff",border:"1px solid #ebebeb",borderRadius:12,overflow:"hidden" }}>
        ...
        {MOCK_CALLS.map((c,i) => (
          <div key={i} style={{ display:"flex",gap:12,alignItems:"center",padding:"13px 20px",borderBottom:i<MOCK_CALLS.length-1?"1px solid #f5f5f5":"none",background:i%2===0?"#fff":"#fafafa" }}>
            <div style={{ width:6,height:6,borderRadius:"50%",background:c.ok?"#222":"#ccc",flexShrink:0 }} />
            <div style={{ flex:1 }}>
      ...
      <div style={{ background:"#fff",border:"1px solid #ebebeb",borderRadius:12,overflow:"hidden" }}>
        ...
        {[
          ...
          <div key={i} style={{ display:"flex",gap:14,alignItems:"center",padding:"14px 20px",borderBottom:"1px solid #f5f5f5",fontFamily:SANS }}>
            <div style={{ width:36,height:36,borderRadius:"50%",background:"#f0f0f0",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:600,color:"#888",fontSize:12,flexShrink:0 }}>{r.name[0]}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:13,fontWeight:500,color:"#111" }}>{r.name}</div>
              <div style={{ fontSize:12,color:"#bbb" }}>{r.svc} · {r.appt}</div>
            </div>
            <span style={{ fontSize:11,background:r.ok===true?"#f0f0f0":r.ok===false?"#f8f8f5":"#f8f8f8",color:r.ok===true?"#444":r.ok===false?"#888":"#bbb",padding:"3px 10px",borderRadius:100,fontWeight:500 }}>{r.status}</span>
          </div>
        ))}
      </div>
    </div>
